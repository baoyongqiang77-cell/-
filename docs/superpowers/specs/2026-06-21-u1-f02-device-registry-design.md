...
- `DeviceStatusService` 只接收 U1-F01 标准 `FlightEvent`，不调用 DJI 接口。
...
| `is_online` | boolean | 非空，默认 `false` |
| `last_seen_at` | datetime/null | 必须为带时区时间 |
| `created_at` | datetime | 非空，UTC |
| `updated_at` | datetime | 非空，UTC |

`device_type` 是 U1-F02 已登记的设备分类，不是生命周期状态。在线状态采用 `is_online` 与 `last_seen_at`，不新增未登记状态枚举。
...
跨字段的设备类型与同租户校验由领域服务执行。数据库外键负责引用完整性，服务负责防止跨租户关联。
...
设备管理 API 可修改名称、厂商、型号和固件版本，不允许直接修改 `is_online` 或 `last_seen_at`。机巢管理 API 可修改同租户的绑定无人机和边缘节点，不允许直接写入遥测产生的环境快照。
...
5. 相同幂等键和相同请求返回原响应；不同请求指纹返回 `IDEMP_409`。

### 7.2 状态同步

1. U1-F01 网关产生标准 `FlightEvent`。
2. `DeviceStatusService` 根据 `device_sn` 在台账中定位设备。
3. `device_bound` 或在线状态事件设置 `is_online=true` 并更新 `last_seen_at`。
4. 明确的离线事件设置 `is_online=false`，保留最后一次在线时间。
5. 机巢环境字段仅写入 `docks.environment_json`，原始回调仍由后续 `flight_events` 能力负责留存。
...
字段格式、设备类型、关联类型和同租户关联错误使用现有统一验证响应，不登记新的业务错误码。
